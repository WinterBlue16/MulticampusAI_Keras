...
search = RandomizedSearchCV(estimator=model, # 모델 선정 
                            param_distributions=hyperparameters, # 사용할 파라미터 값
                            n_iter=10, n_jobs=1, cv=3, verbose=1) # 작업하는 횟수(반복 횟수) # CPU 병렬처리갯수(머신 러닝은 CPU에 민감!,-1=다 쓰겠음) # cv = crossvalidation 수 
