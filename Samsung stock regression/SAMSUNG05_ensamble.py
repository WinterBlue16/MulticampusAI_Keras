...
x1_train_scaled = x1_train.reshape(x1_train.shape[0], 5, 5) 
x1_test_scaled = x1_test.reshape(x1_test.shape[0], 5, 5) 
x2_train_scaled = x2_train.reshape(x2_train.shape[0], 5, 5) 
x2_test_scaled = x2_test.reshape(x2_test.shape[0], 5, 5) 
...
x_dense = LSTM(256, activation='relu')(input1)
...
# 6. 평가예측 
loss, mae = model.evaluate([x1_test_scaled, x2_test_scaled], y1_test, batch_size=5) # loss는 자동적으로 출력
...
y_predict = model.predict([x1_test_scaled, x2_test_scaled], batch_size=5)

def RMSE(y1_test, y_predict): # 실제 정답값, 모델을 통한 예측값 
...
x1_prd = samsung[421:]
x2_prd = kospi200[421:]
x1_prd = x1_prd.reshape(1, 25)
x2_prd = x2_prd.reshape(1, 25)
x1_prd_scaled = scaler.transform(x1_prd)
x2_prd_scaled = scaler.transform(x2_prd)
x1_prd_scaled = x1_prd.reshape(1, 5, 5)
x2_prd_scaled = x2_prd.reshape(1, 5, 5)
result = model.predict([x1_prd_scaled, x2_prd_scaled], batch_size=5)
print(result)
