...
early_stoppping = EarlyStopping(monitor='loss', patience=80, mode='auto')# patience=N, 원하는 값이 나온 후(ex> min, max), 다른 값이 나오지 않은 상태로 N번 지날 경우 멈춤 
model.fit(x_train, y_train, epochs=500, batch_size=1, callbacks=[early_stoppping], validation_data=(x_val, y_val))# early_stopping은 리스트 형식으로 넣어야 함 => [early_stopping] 


#4. 평가예측 
loss, mse = model.evaluate(x_test, y_test, batch_size=100) # loss는 자동적으로 출력
